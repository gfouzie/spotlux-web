...
import { commentsApi, Comment, CommentsResponse, ContentType } from '@/api/comments';
...
export function useComments(
  ...
  const fetchComments = useCallback(
    async (reset: boolean = false) => {
      ...
      try {
        const commentsData = await commentsApi.getComments(contentType, contentId, {
          offset: 0,
          limit: PAGE_SIZE,
        });
  ...
  const addComment = useCallback(
    ...
    [contentType, contentId, data, user]
  );
  ...
  const deleteComment = useCallback(
    async (commentId: number) => {
      ...
      try {
        ...
        // Make API call
        await commentsApi.deleteComment(contentType, contentId, commentId);
      } catch (err) {
        // Rollback on error
        setData(previousData);
        setError(
  ...
  const likeComment = useCallback(
    async (commentId: number) => {
      ...
      try {
        ...
        // Make API call
        await commentsApi.likeComment(contentType, contentId, commentId);
      } catch (err) {
        // Rollback on error
        setData(previousData);
  ...
  const unlikeComment = useCallback(
    async (commentId: number) => {
      ...
      try {
        ...
        // Make API call
        await commentsApi.unlikeComment(contentType, contentId, commentId);
      } catch (err) {
        // Rollback on error
        setData(previousData);
        setError(
  ...
  const loadMore = useCallback(async () => {
    if (!contentId || !data || isLoading) return;
    ...
    try {
      const moreComments = await commentsApi.getComments(contentType, contentId, {
        offset: nextOffset,
        limit: PAGE_SIZE,
      });
  ...
  }, [contentType, contentId, data, isLoading, offset]);
